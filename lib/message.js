 ...
 */
export function sanitizeErrorText(text, maxLen = 300) {
    let s = String(text);
 ...
 */
export function buildSystemCommand(platform, title, message, options) {
    if (platform === "win32") {
...
/**
 * 跨 dsh 0.1.1 / 0.1.2 读取会话事件日志（特性检测）。
 * 0.1.2-rc.1 起 Session 移除了 `events` getter，公共读入口改为
 * snapshotEvents()/ownEvents()；0.1.1 无 snapshotEvents 方法——按方法存在性
 * 双兼容。入参来自事件 payload（跨宿主边界，不受信），故运行时显式收窄。
 * @param session 会话对象（agent.session）。
 * @returns 只读事件数组；无法读取时 undefined（调用方按无证据处理）。
 */
function sessionEventsOf(session) {
    const s = session;
    try {
        if (typeof s?.snapshotEvents === "function") {
            // dsh 0.1.2+：不可变快照入口（此处 Array.isArray 只做运行时守卫，
            // 不承担类型收窄——类型收窄由调用点的显式 cast 负责）
            const snap = s.snapshotEvents();
            return Array.isArray(snap) ? snap : undefined;
        }
        // dsh 0.1.1：events getter
        return Array.isArray(s?.events) ? s.events : undefined;
    }
    catch {
        return undefined;
    }
 ...
 */
export function sessionTitleOf(agent) {
    try {
 ...
 */
export function lastTurnEndOf(agent) {
    try {
        ...
        for (let i = events.length - 1; i >= 0; i -= 1) {
            ...
            // （复核闸 P1）。
            if (typeof ev.data?.turn !== "number" || !Number.isFinite(ev.data.turn))
                continue;
 ...
 *   前一致。
 */
export function isSubagentOf(agent, ownership) {
    ...
    if (parentId === undefined || ownership === undefined || selfId === undefined)
        return false;
...
export const NOTIFY_KINDS = {
    ask: {
        ...
        message: (detail) => {
            ...
            if (detail.remindMinutes)
                lines.push(`已等待 ${detail.remindMinutes} 分钟，仍在等待你的审批`);
    ...
    error: {
        ...
        message: (detail) => {
            ...
            if (detail.mergedCount)
                lines.push(`（窗口内另有 ${detail.mergedCount} 条同类错误）`);
            if (detail.mergedErrors && detail.mergedErrors.length > 0)
                lines.push(`窗口内其他错误：${detail.mergedErrors.join(" / ")}`);
            return lines.join("\n");
        },
    },
    "turn-end": {
