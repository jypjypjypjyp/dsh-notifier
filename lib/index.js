...
export async function apply(ctx, config = {}) {
     ...
     * 用户暂停/中断（turn/end reason aborted）固定静默。
     */
    const agentStates = new Map(); // agentId -> { runningSeen, startedAt, lastEndedTurn }
    /**
    ...
    function hookUserQuestions() {
        ...
        if (!svc || typeof svc.ask !== "function")
            return;
            ...
            try {
                if (current.notifyQuestion) {
                    ...
                    notify("question", {
                        ...
                        taskTitle: sessionTitleOf(request?.agent),
                        // 提问文本同样可能内嵌上下文片段，进通知前脱敏（120 字符截断）
                        question: first && first.question ? sanitizeErrorText(first.question, 120) : undefined,
                    });
                }
            }
            catch (error) {
     ...
     */
    let doneBatch = null;
    let doneBatchTimer = null;
    function enqueueDone(kind, title, durationMs) {
        ...
        if (doneBatch.titles.length > 4)
            doneBatch.titles = doneBatch.titles.slice(-4);
    }
     ...
     */
    const eventStreamEnds = new Map(); // sessionId -> 最新推送 turn/end
    disposers.push(ctx.on("session/event", (session, event) => {
        try {
            ...
            if (!Number.isFinite(turn))
                return; // 非有限 turn 的证据不可用：直接 skip 不落记忆，
    ...
    disposers.push(ctx.on("agent/status", ({ agent, status }) => {
        try {
            ...
            if (status === "idle" && state.runningSeen) {
                ...
                const best = streamed !== undefined && (ended === undefined || (Number.isFinite(streamed.turn) && streamed.turn > ended.turn))
                    ? streamed
                    : ended;
                const rememberedTurn = state.lastEndedTurn;
                const hasNewEnd = best !== undefined && (rememberedTurn === undefined || best.turn > rememberedTurn);
                if (best !== undefined)
                    state.lastEndedTurn = best.turn;
                ...
                if (!hasNewEnd || best === undefined || best.kind !== "completed") {
                    ...
                    ctx.logger.warn(`dsh-notifier: 完成判定跳过（不发 done）agent=${agentId} kind=${best?.kind ?? "none"} ` +
                ...
                if (_isSub) {
                    if (current.notifySubagentDone)
                        enqueueDone("subagent-done", taskTitle, durationMs);
                }
    ...
    disposers.push(ctx.on("agent/error", (payload) => {
        try {
            ...
            if (prev !== undefined && now - prev.since < mergeMs) {
                ...
                if (prev.lastMessages.length > 2)
                    prev.lastMessages.shift();
    ...
    disposers.push(ctx.on("agent/turn-stopping", async (payload) => {
        try {
            if (current.notifyTurnEnd) {
                ...
                if (turnNotified.has(key))
                    return; // 同一 (agent, talk) 只通知一次，防刷屏
    ...
    ctx.effect(() => () => {
        ...
        for (const t of askRemindTimers.values())
            clearTimeout(t);
