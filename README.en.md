...
`dsh-session`, `dsh-host-webserver`, ...): **`import type` only, compile-time usage** —
build artifacts contain zero official runtime imports; all runtime objects are injected
by the dsh host. These type layers are **not declared as npm dependencies** — the public
npm registry cannot resolve DSH's internal pre-release type packages (e.g.
`@deepseek-ai/dsh-type-meta`), so `scripts/build.sh` junction-links them from a local DSH
install (same versions the runtime uses) at build time. Consumers running type checks
against the plugin need a DSH install (or set `DSH_CORE` to a node_modules containing
`@deepseek-ai/dsh-agent`); skipping type checking is unaffected.
...
# Source is in src/, must build after changes (standalone repo: pnpm install, then build)
pnpm install
pnpm build
node test/smoke.ts
```
