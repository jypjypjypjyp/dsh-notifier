...
/**
 * 递归删除临时目录（测试清理用）。
 * Node 24 + macOS 下 fs.rmSync recursive 对仍有文件写入/重命名进行中的目录会偶发
 * ENOTEMPTY（异步 history 追加与 finally 清理竞态）。真异步重试（setTimeout 让步，
 * 让在途的 rename/write settle）直至成功，保证 `node test/smoke.ts` deterministic。
 * 失败仅抛最后一次错误，不中断主流程。调用方（各测试文件 finally）须 await。
 */
export async function rmrf(dir) {
  ...
  for (let attempt = 0; attempt < 10; attempt += 1) {
    try {
      ...
      lastErr = e;
      // 真让步（非忙等）：让在途的异步历史 rename / write 完成
