...
/**
 * smoke-lib — 共享 smoke 工具（本地化，自包含）。
 *
 * 提供：
 *   - check(name, fn) / report(title)：断言注册 + 汇总 + 退出码；
 *   - fakeReq / makeRes：路由 handler 测试用 req/res stub；
 *   - assertClientProductContract：客户端产物「执行契约」断言（复用 client-contract-lib）；
 *   - assertClientSourceContract：客户端产物「源形态」契约断言（兼容 legacy 手写 IIFE 与
 *     wrapper 生成两种产物形态）；
 *   - clientRouteLiterals：提取客户端产物中的字符串字面量。
 */
...
/** 客户端产物执行契约断言（复用 client-contract-lib）。
...
export function assertClientSourceContract(pkgDir) {
  ...
  // 产物经 esbuild cjs __export 装配——功能契约由 assertClientProductContract 硬保证。
