...
import { assert, makeNotifier, agentWithTitle, fakeAgents, waitMergeWindow, fakeReq, makeRes, turnEndEvent , rmrf } from "./helpers.ts";
...
try {
  ...
  // 错误合并窗口过期后通知并携带合并计数（独立实例：20ms 窗口）
  {
    ...
    writeFileSync(mergeCfg, JSON.stringify({ errorMergeWindowMs: 200 }));
    const infos = [];
