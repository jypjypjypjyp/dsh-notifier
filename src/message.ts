 ...
 */
function sessionEventsOf(session: unknown): ReadonlyArray<unknown> | undefined {
  ...
  try {
    ...
    return Array.isArray(s?.events) ? (s.events as ReadonlyArray<unknown>) : undefined;
  } catch {
    return undefined;
  }
}
...
export function sessionTitleOf(agent: Agent | undefined): string | undefined {
  try {
    ...
    const events = sessionEventsOf(agent?.session) as ReadonlyArray<{ type: unknown; data?: { title?: unknown } }> | undefined;
...
export function lastTurnEndOf(agent: Agent | undefined): { turn: number; kind: TurnEndKind } | undefined {
  try {
    ...
    const events = sessionEventsOf(agent?.session) as ReadonlyArray<{ type: unknown; data?: { turn?: unknown; reason?: unknown } }> | undefined;
    if (events === undefined) return undefined;
    for (let i = events.length - 1; i >= 0; i -= 1) {
